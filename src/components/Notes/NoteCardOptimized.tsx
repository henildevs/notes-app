...
const NoteCard: React.FC<NoteCardProps> = memo(({
  ...
  }, [note.isEncrypted, note.content, note.plainTextContent, truncateText]);
  ...
  const wordCount = useMemo(() => {
    return note.content ? (note.plainTextContent?.split(/\s+/).filter(w => w).length || 0) : 0;
  }, [note.content, note.plainTextContent]);

  // Memoized formatted date
  const formattedDate = useMemo(() => formatDate(note.updatedAt), [note.updatedAt, formatDate]);

  // Memoized preview text
  const previewText = useMemo(() => getPreviewText(), [getPreviewText]);

  // Memoized display title
  const displayTitle = useMemo(() => getDisplayTitle(), [getDisplayTitle]);
